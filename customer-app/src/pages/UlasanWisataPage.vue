...
import {UlasanWisataRequest} from "../model/request/ulasan_wisata_request.js";
...
async function kirimUlasan(e) {
  ...
  if (useUserStore.user.userId !== 0) {
    const request = new UlasanWisataRequest(parseInt(useWisataStore.state.singleData.id), parseInt(useUserStore.user.biodata.id), parseFloat(rating.value), komentar.value);

    await WisataService.CreateUlasan(request).then((response) => {
      // Success
      if (response === null) {
...
const checkUlasan = computed(() => {
  ...
  useWisataStore.state.singleData?.ulasan.filter(item => {
